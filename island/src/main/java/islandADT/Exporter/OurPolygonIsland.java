...
public class OurPolygonIsland{
    public Structs.Polygon create_geometry(List<Integer> segments, TileTypeWrapper tileType, int centroid, List<Integer> neighbours, List<float[]> coords, int height, double moisture) {
        ...
        Structs.Polygon polygon = Structs.Polygon.newBuilder().addAllSegmentIdxs(segments).setCentroidIdx(centroid).addAllNeighborIdxs(neighbours).addProperties(color).addProperties(x_coords).addProperties(y_coords).addProperties(height_p).addProperties(moisture_p).build();
        return polygon;
    }
